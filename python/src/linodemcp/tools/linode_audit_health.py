...
def audit_health_result(_arguments: dict[str, Any]) -> list[TextContent]:
    """Collect audit subsystem status and return it as proto-canonical JSON.

    A store the collector cannot read answers as a tool-result error, the way
    the Go hook's does, rather than raising out of the handler.
    """
    try:
        report = collect_health(audit_sqlite_path(), resolve_default_audit_dir())
    except OSError as exc:
        return error_response(f"failed to collect audit health: {exc}")
