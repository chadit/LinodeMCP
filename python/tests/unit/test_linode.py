...
async def test_list_maintenance_policies_sends_exact_route() -> None:
    """Maintenance policy listing sends GET /maintenance/policies."""
    client = Client("https://api.linode.com/v4", "test-token")
    response_data: dict[str, Any] = {
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        result = await client.list_maintenance_policies()

    assert result == response_data
    mock_request.assert_called_once_with("GET", "/maintenance/policies")
    await client.close()


async def test_list_maintenance_policies_wraps_http_errors() -> None:
    """Maintenance policy listing wraps HTTP errors."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        with pytest.raises(NetworkError) as excinfo:
            await client.list_maintenance_policies()

    assert "ListMaintenancePolicies" in str(excinfo.value)
    await client.close()


async def test_retryable_list_maintenance_policies_delegates_to_client() -> None:
    """RetryableClient delegates maintenance policy listing to Client."""
    retryable = RetryableClient("https://api.linode.com/v4", "test-token")

    with patch.object(
        retryable.client, "list_maintenance_policies", new_callable=AsyncMock
    ) as mock_list:
        mock_list.return_value = {"data": []}
...
async def test_delete_instance_config() -> None:
    """Delete Linode instance config sends DELETE to the exact route."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        await client.delete_instance_config(123, 6)

    mock_request.assert_called_once_with("DELETE", "/linode/instances/123/configs/6")
    await client.close()
...
async def test_delete_instance_config_escapes_path_separators(
    linode_id: Any, config_id: Any, expected: str
) -> None:
    """Linode instance config delete escapes path separators and query markers."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    mock_request.assert_called_once_with("DELETE", expected)
    await client.close()


async def test_delete_instance_config_wraps_http_errors() -> None:
    """Delete Linode instance config wraps HTTP errors."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    assert "DeleteInstanceConfig" in str(excinfo.value)
    await client.close()


async def test_retryable_delete_instance_config_delegates_without_retry() -> None:
    """RetryableClient delegates destructive config delete without replay retry."""
    retryable = RetryableClient("https://api.linode.com/v4", "test-token")

    with patch.object(
        retryable.client, "delete_instance_config", new_callable=AsyncMock
    ) as mock_delete:
        mock_delete.side_effect = [httpx.HTTPError("boom"), None]

        with pytest.raises(httpx.HTTPError):
            await retryable.delete_instance_config(123, 6)

    mock_delete.assert_awaited_once_with(123, 6)
    await retryable.close()
...
async def test_list_instance_config_interfaces_rejects_non_object_items() -> None:
    ...
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = [{"id": 202, "purpose": "vpc"}, "invalid"]
...
async def test_list_managed_credentials_sends_get_to_managed_credentials_route() -> (
    None
):
    """Test Managed credentials listing sends documented GET query."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        result = await client.list_managed_credentials(page=2, page_size=25)

    assert result == response_data
    mock_request.assert_called_once_with(
        "GET", "/managed/credentials?page=2&page_size=25"
    )
    await client.close()


async def test_list_managed_credentials_sends_exact_route_without_query_or_body() -> (
    None
):
    """Test Managed credentials listing omits query and body when unset."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        result = await client.list_managed_credentials()

    assert result == {"data": [], "page": 1, "pages": 1, "results": 0}
    mock_request.assert_called_once_with("GET", "/managed/credentials")
    await client.close()


async def test_list_managed_credentials_wraps_http_errors() -> None:
    """Test Managed credentials listing wraps HTTP errors."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        with pytest.raises(NetworkError) as excinfo:
            await client.list_managed_credentials()

    assert "ListManagedCredentials" in str(excinfo.value)
    await client.close()
...
async def test_retryable_list_managed_credentials_delegates_to_client() -> None:
    ...
    with patch.object(
        retryable.client, "list_managed_credentials", new_callable=AsyncMock
    ) as mock_list:
        mock_list.return_value = {"data": [], "page": 1, "pages": 1, "results": 0}
...
async def test_disable_managed_service_sends_post_to_disable_route() -> None:
    """Managed service disable sends POST /managed/services/{id}/disable."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = mock_response
        result = await client.disable_managed_service(9944)

    assert result == response_data
    mock_request.assert_called_once_with("POST", "/managed/services/9944/disable")
    await client.close()


@pytest.mark.parametrize("service_id", [0, -1, "123", "1/2", "1?x", "..", True])
async def test_disable_managed_service_rejects_bad_service_id_before_request(
    service_id: object,
) -> None:
    """Managed service disable validates service_id before dispatch."""
    client = Client("https://api.linode.com/v4", "test-token")
    with (
        patch.object(client, "make_request", new_callable=AsyncMock) as mock_request,
        pytest.raises(ValueError, match="service_id"),
    ):
        await client.disable_managed_service(cast("Any", service_id))
    mock_request.assert_not_called()
    await client.close()


async def test_disable_managed_service_wraps_http_errors() -> None:
    """Managed service disable maps HTTP errors to DisableManagedService."""
    client = Client("https://api.linode.com/v4", "test-token")
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        mock_request.side_effect = httpx.HTTPError("boom")
        with pytest.raises(NetworkError) as excinfo:
            await client.disable_managed_service(9944)
    assert "DisableManagedService" in str(excinfo.value)
    await client.close()


async def test_retryable_disable_managed_service_delegates_once_without_retry() -> None:
    """RetryableClient does not replay Managed service disable on errors."""
    retryable = RetryableClient("https://api.linode.com/v4", "test-token")
    mock_disable = AsyncMock(side_effect=httpx.HTTPError("boom"))
    object.__setattr__(retryable.client, "disable_managed_service", mock_disable)

    try:
        with pytest.raises(httpx.HTTPError):
            await retryable.disable_managed_service(9944)
    finally:
        await retryable.close()

    mock_disable.assert_awaited_once_with(9944)
...
async def test_delete_managed_contact_sends_delete_to_managed_contact_route() -> None:
    """Managed contact delete sends DELETE to the documented route."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        result = await client.delete_managed_contact(123)

    assert result == response_data
    mock_request.assert_called_once_with("DELETE", "/managed/contacts/123")
    await client.close()


@pytest.mark.parametrize("contact_id", [0, "1/2", "1?x", "..", True])
async def test_delete_managed_contact_rejects_invalid_contact_id(
    contact_id: Any,
) -> None:
    """Managed contact delete validates finite integer IDs before dispatch."""
    client = Client("https://api.linode.com/v4", "test-token")
...
async def test_delete_managed_contact_wraps_http_errors() -> None:
    """Managed contact delete maps HTTP errors to DeleteManagedContact."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        with pytest.raises(NetworkError) as excinfo:
            await client.delete_managed_contact(123)

    assert "DeleteManagedContact" in str(excinfo.value)
    await client.close()


async def test_retryable_delete_managed_contact_delegates_once_without_retry() -> None:
    """RetryableClient does not replay Managed contact deletion on errors."""
    retryable = RetryableClient("https://api.linode.com/v4", "test-token")

    with patch.object(
        retryable.client, "delete_managed_contact", new_callable=AsyncMock
    ) as mock_delete:
        mock_delete.side_effect = httpx.HTTPError("boom")

        with pytest.raises(httpx.HTTPError):
            await retryable.delete_managed_contact(123)
...
async def test_list_managed_services_sends_get_to_managed_services_route() -> None:
    """Managed services listing sends documented GET route."""
    client = Client("https://api.linode.com/v4", "test-token")
    response_data: dict[str, Any] = {
        "data": [
            {"id": 123, "label": "web-1", "service_type": "url"},
            {"id": 456, "label": "db-1", "service_type": "tcp"},
        ],
        "page": 1,
        "pages": 1,
        "results": 2,
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        result = await client.list_managed_services()

    assert result == response_data
    mock_request.assert_called_once_with("GET", "/managed/services")
    await client.close()


async def test_list_managed_services_wraps_http_errors() -> None:
    """Managed services listing wraps HTTP errors."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        ...
        with pytest.raises(NetworkError) as excinfo:
            await client.list_managed_services()

    assert "ListManagedServices" in str(excinfo.value)
    await client.close()
...
async def test_retryable_list_managed_services_delegates_to_client() -> None:
    ...
    with patch.object(
        retryable.client, "list_managed_services", new_callable=AsyncMock
    ) as mock_list:
        mock_list.return_value = {"data": [], "page": 1, "pages": 1, "results": 0}
...
async def test_retryable_client_share_ipv4s_does_not_replay_errors() -> None:
    """State-changing IPv4 sharing is delegated once without replay retry."""
    client = RetryableClient(
        "https://api.linode.com/v4", "test-token", RetryConfig(max_retries=3)
    )
    ...
    with patch.object(
        ...
        mock_share.side_effect = NetworkError(
            "ShareIPv4s", httpx.ConnectError("connection refused")
        )
        with pytest.raises(NetworkError):
            await client.share_ipv4s(["192.168.1.1"], 12345)
...
async def test_list_longview_clients_sends_get_to_longview_clients_route() -> None:
    ...
    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = response
        result = await client.list_longview_clients()

    assert result == {"data": [{"id": 123}], "page": 1, "pages": 1}
    mock_request.assert_awaited_once_with("GET", "/longview/clients")
    await client.close()


async def test_list_longview_clients_includes_pagination_query_params() -> None:
    """Longview clients list includes page and page_size query params."""
    client = Client("https://api.linode.com/v4", "test-token")
    response = httpx.Response(200, json={"data": [], "page": 2, "pages": 3})

    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = response
        await client.list_longview_clients(page=2, page_size=50)

    mock_request.assert_awaited_once_with(
        "GET", "/longview/clients?page=2&page_size=50"
    )
    await client.close()
...
@pytest.mark.parametrize(
    ...
    [
        (0, None, "page must be a positive integer"),
        (None, 24, "page_size must be between 25 and 500"),
        (None, 501, "page_size must be between 25 and 500"),
        ("2", None, "page must be a positive integer"),
        (None, True, "page_size must be between 25 and 500"),
    ],
)
async def test_list_longview_clients_rejects_invalid_pagination(
    page: object, page_size: object, message: str
) -> None:
    """Longview clients list rejects invalid pagination before dispatch."""
    client = Client("https://api.linode.com/v4", "test-token")
    ...
    with (
        ...
        await client.list_longview_clients(
            page=cast("Any", page), page_size=cast("Any", page_size)
        )
...
async def test_list_longview_clients_wraps_http_errors() -> None:
    """Longview clients list maps HTTP errors to ListLongviewClients."""
    client = Client("https://api.linode.com/v4", "test-token")

    with patch.object(client, "make_request", new_callable=AsyncMock) as mock_request:
        mock_request.side_effect = httpx.ReadTimeout("timeout")
        with pytest.raises(NetworkError, match="ListLongviewClients"):
            await client.list_longview_clients()
...
async def test_retryable_client_list_longview_clients_delegates() -> None:
    ...
    with patch.object(
        client.client, "list_longview_clients", new_callable=AsyncMock
    ) as mock_list:
        mock_list.return_value = {"data": [{"id": 123}]}
        result = await client.list_longview_clients(page=2, page_size=50)
