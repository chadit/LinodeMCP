...
The shared behavior fixture in ``testdata/behavior/linode_domain_create.json``
pins what both languages must agree on. This file covers the Python-local
half: registration, the schema the tool advertises, and the response shapes
the handler accepts or rejects.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"domain": "example.com", "type": "slave", "master_ips": ["192.0.2.1", 2]},
...
async def test_domain_create_validates_frozen_contract(
    ...
    """Each documented constraint rejects with its own message."""
...
async def test_domain_create_requires_literal_true_confirmation(
    ...
    """A truthy non-boolean is not consent for a create."""
    result = await handle_linode_domain_create(
        {**_MASTER_ARGS, "confirm": 1}, sample_config
    )
...
async def test_domain_create_forwards_exact_full_body_and_route(
    ...
    """Every explicitly supplied optional reaches the wire, zero values included."""
    client = _client_with_response({"id": 7, "domain": "example.com", "type": "slave"})
    arguments = {
        "domain": "example.com",
        "type": "slave",
        "axfr_ips": ["192.0.2.10"],
...
async def test_domain_create_omits_absent_optional_fields(
    ...
    """An unsupplied optional stays out of the body so the API default applies."""
...
async def test_domain_create_dry_run_previews_exact_request(
    ...
    """The preview body is the body the live call would send."""
    arguments = {
        "domain": "example.com",
        "type": "slave",
        "description": "",
...
async def test_domain_create_rejects_incompatible_success_response_shapes(
    ...
    """A success body that is not a decodable domain object is an error."""
...
@pytest.mark.parametrize("response", [{}, {"future_field": "kept by the API"}])
async def test_domain_create_preserves_compatible_legacy_response_shapes(
    sample_config: Config, response: Any
) -> None:
    """An object with only unknown fields still decodes, matching Go."""
    client = _client_with_response(response)
    with patch("linodemcp.tools.helpers.RetryableClient", return_value=client):
        result = await handle_linode_domain_create(dict(_MASTER_ARGS), sample_config)
