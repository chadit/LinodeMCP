...
request. ``api_status`` optionally sets the response status and defaults to
200. A case whose args include ``dry_run: true`` additionally asserts every
captured request is a GET: a dry run may read whatever it needs for its preview
...
async def test_behavior_conformance(
    ...
    api_status: int = case.get("api_status", 200)
