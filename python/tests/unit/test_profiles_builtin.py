...
def test_managed_tools_are_account_scoped() -> None:
    ...
    assert categories("linode_managed_credential_create") == ["account"]
    assert required_scopes("linode_managed_credential_create", Capability.Write) == [
        Scope.AccountReadWrite
    ]
    assert categories("linode_managed_credential_revoke") == ["account"]
    ...
    assert categories("linode_managed_service_update") == ["account"]
    ...
    assert categories("linode_managed_service_get") == ["account"]
    assert required_scopes("linode_managed_service_get", Capability.Read) == [
        Scope.AccountReadOnly
    ]
    assert categories("linode_managed_credential_username_password_update") == [
        "account"
    ]
    assert required_scopes(
        "linode_managed_credential_username_password_update", Capability.Write
    ) == [Scope.AccountReadWrite]
    assert categories("linode_managed_service_delete") == ["account"]
    assert required_scopes("linode_managed_service_delete", Capability.Destroy) == [
        Scope.AccountReadWrite
    ]
