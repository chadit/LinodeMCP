...
func TestClientCreateMonitorServiceTokenSuccess(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, monitorServiceTokenPath, r.URL.Path, "request path should match")
		...
		assert.Equal(t, []any{float64(10), float64(20)}, body["entity_ids"])
	...
	assert.Equal(t, "monitor-token", got.Token)
}
...
func TestClientCreateMonitorServiceTokenEscapesPathParams(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/monitor/services/dbaas%2Fpostgres/token", r.URL.EscapedPath(), "request path should be escaped")
		w.Header().Set("Content-Type", "application/json")
		assert.NoError(t, json.NewEncoder(w).Encode(map[string]any{keyToken: "monitor-token"}))
	}))
	t.Cleanup(srv.Close)
...
func TestClientCreateMonitorServiceTokenDoesNotRetryTransientError(t *testing.T) {
	...
	require.Error(t, err)
	assert.Nil(t, got)
	assert.Equal(t, int32(1), calls.Load(), "token creation must not retry after transient failure")
}
