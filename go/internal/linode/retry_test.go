...
func TestNewRetryableClient(t *testing.T) {
	...
	rc := linode.NewRetryableClient("https://api.example.com", "tok", linode.DefaultRetryConfig())
	assert.NotNil(t, rc.Client)
	assert.Equal(t, 3, rc.RetryConfigField().MaxRetries)
}

func TestNewRetryableClientWithDefaults(t *testing.T) {
	t.Parallel()

	rc := linode.NewRetryableClientWithDefaults("https://api.example.com", "tok")
	assert.NotNil(t, rc.Client)
	assert.Equal(t, linode.DefaultRetryConfig().MaxRetries, rc.RetryConfigField().MaxRetries)
}
...
func TestRetryableClient_GetProfile_SuccessNoRetry(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		require.NoError(t, json.NewEncoder(w).Encode(linode.Profile{Username: "user1"}))
	}))
	defer srv.Close()

	rc := linode.NewRetryableClient(srv.URL, "token", linode.RetryConfig{
		MaxRetries:    3,
	...
	profile, err := rc.GetProfile(t.Context())
...
func TestRetryableClient_RetriesOnServerError(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		require.NoError(t, json.NewEncoder(w).Encode(linode.Profile{Username: "recovered"}))
	}))
	defer srv.Close()
...
func TestRetryableClient_NoRetryOnAuthError(t *testing.T) {
	...
	rc := linode.NewRetryableClient(srv.URL, "bad-token", linode.RetryConfig{
		MaxRetries:    3,
		BaseDelay:     1 * time.Millisecond,
		MaxDelay:      10 * time.Millisecond,
		BackoffFactor: 2.0,
...
func TestRetryableClient_ListInstances_Retries(t *testing.T) {
	...
	instances, err := rc.ListInstances(t.Context())
	require.NoError(t, err)
...
func TestRetryableClient_GetInstance_Retries(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		require.NoError(t, json.NewEncoder(w).Encode(linode.Instance{ID: 99, Label: "recovered"}))
	}))
	defer srv.Close()
	...
	instance, err := rc.GetInstance(t.Context(), 99)
	require.NoError(t, err)
...
func TestShouldRetry_ForbiddenNotRetryable(t *testing.T) {
	...
	rc := linode.NewRetryableClientWithDefaults("http://x", "t")
	assert.False(t, rc.ShouldRetry(&linode.APIError{StatusCode: 403, Message: "forbidden"}))
}
...
func TestShouldRetry_NetworkErrorRetryable(t *testing.T) {
	...
	rc := linode.NewRetryableClientWithDefaults("http://x", "t")
	assert.True(t, rc.ShouldRetry(&linode.NetworkError{Operation: "test", Err: fmt.Errorf("conn")}))
}

func TestCalculateDelay_RespectMaxDelay(t *testing.T) {
	t.Parallel()

	rc := linode.NewRetryableClient("http://x", "t", linode.RetryConfig{
		MaxRetries:    5,
