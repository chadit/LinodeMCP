...
func TestClientUpdateFirewallSettingsSuccess(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !reflect.DeepEqual(body["default_firewall_ids"], map[string]int{firewallSettingsKeyLinode: linodeDefaultID, "nodebalancer": nodeBalancerDefaultID, "public_interface": publicInterfaceDefaultID, "vpc_interface": vpcInterfaceDefaultID}) {
			t.Errorf("got %v, want %v", body["default_firewall_ids"], map[string]int{firewallSettingsKeyLinode: linodeDefaultID, "nodebalancer": nodeBalancerDefaultID, "public_interface": publicInterfaceDefaultID, "vpc_interface": vpcInterfaceDefaultID})
		}
	...
	if result.DefaultFirewallIDs.Linode != linodeDefaultID {
		t.Errorf("result.DefaultFirewallIDs.Linode = %v, want %v", result.DefaultFirewallIDs.Linode, linodeDefaultID)
	}

	if result.DefaultFirewallIDs.NodeBalancer != nodeBalancerDefaultID {
		t.Errorf("result.DefaultFirewallIDs.NodeBalancer = %v, want %v", result.DefaultFirewallIDs.NodeBalancer, nodeBalancerDefaultID)
	}

	if result.DefaultFirewallIDs.PublicInterface != publicInterfaceDefaultID {
		t.Errorf("result.DefaultFirewallIDs.PublicInterface = %v, want %v", result.DefaultFirewallIDs.PublicInterface, publicInterfaceDefaultID)
	}

	if result.DefaultFirewallIDs.VPCInterface != vpcInterfaceDefaultID {
		t.Errorf("result.DefaultFirewallIDs.VPCInterface = %v, want %v", result.DefaultFirewallIDs.VPCInterface, vpcInterfaceDefaultID)
	}
}
