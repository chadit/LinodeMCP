...
func TestClientUpdateInstanceInterfaceSuccess(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		checkEqual(t, 789, got.VPC.SubnetID, "subnet id should match")
		...
		checkNoError(t, json.NewEncoder(w).Encode(updated), "encoding response should not fail")
	}))
	t.Cleanup(srv.Close)
	...
	requireNoError(t, err, "UpdateInstanceInterface should succeed on 200 response")
	requireNotNil(t, got)
	checkEqual(t, 456, got.ID)
	requireNotNil(t, got.VPC)
	checkEqual(t, 789, got.VPC.SubnetID)
}
...
func TestClientUpdateInstanceInterfaceRejectsInvalidInput(t *testing.T) {
	...
	requireErrorIs(t, err, linode.ErrInterfaceIDPositive)

	_, err = client.UpdateInstanceInterface(t.Context(), 123, 456, nil)
	requireErrorIs(t, err, linode.ErrUpdateInstanceInterfaceRequestRequired)
...
func TestClientUpdateInstanceInterfaceDoesNotReplayTransientPut(t *testing.T) {
	...
	checkEqual(t, int32(1), calls.Load(), "PUT update call should not be replayed after transient server error")
}
