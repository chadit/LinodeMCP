...
func TestLinodePlacementGroupCreateTool(t *testing.T) {
	...
	t.Run("definition", func(t *testing.T) {
		...
		checkEqual(t, placementGroupCreateToolName, tool.Name, "tool name should match")
		checkEqual(t, profiles.CapWrite, capability, "placement group creation should be CapWrite")
		...
		expectContains(t, props, keyPlacementGroupTypeJSON, "schema should include placement_group_type")
		expectContains(t, props, keyPlacementGroupPolicyJSON, "schema should include placement_group_policy")
		...
		expectContains(t, tool.InputSchema.Required, "label", "label must be marked required")
		expectContains(t, tool.InputSchema.Required, "region", "region must be marked required")
		expectContains(t, tool.InputSchema.Required, keyPlacementGroupTypeJSON, "placement_group_type must be marked required")
		expectContains(t, tool.InputSchema.Required, keyPlacementGroupPolicyJSON, "placement_group_policy must be marked required")
	...
	t.Run("dry_run returns preview without client call", func(t *testing.T) {
		...
		expectContains(t, textContent.Text, `"path": "/placement/groups"`)
		expectContains(t, textContent.Text, "side_effects")
		expectContains(t, textContent.Text, "will be created in region")
		checkEqual(t, int32(0), calls, "create dry_run must not call the Linode API")
	})
	...
	t.Run("success", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			checkEqual(t, placementGroupCreateLabel, got.Label)
			checkEqual(t, placementGroupCreateRegion, got.Region)
			checkEqual(t, placementGroupType, got.PlacementGroupType)
			checkEqual(t, placementGroupCreatePolicy, got.PlacementGroupPolicy)

			w.Header().Set("Content-Type", "application/json")
			checkNoError(t, json.NewEncoder(w).Encode(linode.PlacementGroup{ID: 123, Label: placementGroupCreateLabel, Region: placementGroupCreateRegion, PlacementGroupType: placementGroupType, PlacementGroupPolicy: placementGroupCreatePolicy, IsCompliant: true}))
		}))
		defer srv.Close()
		...
		textContent, ok := result.Content[0].(mcp.TextContent)
		expectTrue(t, ok, "content should be TextContent")
		expectContains(t, textContent.Text, placementGroupCreateLabel, "response should include label")
		expectContains(t, textContent.Text, placementGroupCreateRegion, "response should include region")
	})
}
