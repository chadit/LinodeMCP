...
func TestLinodeImageShareGroupsByImageListToolDefinition(t *testing.T) {
	...
	if tool.Name != "linode_image_sharegroups_by_image_list" {
		t.Errorf("tool.Name = %v, want %v", tool.Name, "linode_image_sharegroups_by_image_list")
	}
...
func TestLinodeImageShareGroupsByImageListToolClientError(t *testing.T) {
	...
	if !strings.Contains(textContent.Text, "Failed to retrieve image share groups by image") {
		t.Errorf("textContent.Text does not contain %v", "Failed to retrieve image share groups by image")
	}
}
