...
func TestLinodeDomainCreateContract(t *testing.T) {
	...
	for _, field := range []string{
		"environment", keyDomain, keyType, keyAXFRIPs, keyDescription, keyExpireSec,
	...
	for field, want := range map[string][]string{
		...
		if err := json.Unmarshal(schema.Properties[field], &property); err != nil {
			t.Fatalf("unmarshal %s schema: %v", field, err)
		}
...
func TestLinodeDomainCreateForwardsOptionalBody(t *testing.T) {
	...
	wouldExecute, ok := preview["would_execute"].(map[string]any)
	if !ok {
		t.Fatalf("would_execute = %T, want object", preview["would_execute"])
	}

	if got := wouldExecute["method"]; got != "POST" {
		t.Errorf("would_execute.method = %v, want POST", got)
	}
	...
	if gotBody, ok := wouldExecute["body"].(map[string]any); !ok || !reflect.DeepEqual(gotBody, wantBody) {
		t.Errorf("would_execute.body = %#v, want %#v", gotBody, wantBody)
	}
}
