...
func TestLinodeLongviewSubscriptionsTool(t *testing.T) {
	t.Parallel()
	...
	t.Run("definition", func(t *testing.T) {
		...
		assert.Equal(t, "linode_longview_subscriptions", tool.Name, "tool name should match")
	...
	t.Run("success", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			assert.Equal(t, longviewSubscriptionsToolPath, r.URL.Path, "request path should match")
			assert.Equal(t, longviewSubscriptionsToolQuery, r.URL.RawQuery, "request query should include pagination")
		...
		assert.Contains(t, textContent.Text, longviewSubscriptionLabel, "response should contain subscription label")
	})
	...
	t.Run("api error", func(t *testing.T) {
		...
		textContent, ok := result.Content[0].(mcp.TextContent)
		require.True(t, ok, "content should be TextContent")
		assert.Contains(t, textContent.Text, "Failed to retrieve linode_longview_subscriptions", "response should identify failed tool")
