...
func TestLinodeImageShareGroupImagesListTool(t *testing.T) {
	...
	t.Run("definition", func(t *testing.T) {
		...
		assert.Equal(t, "linode_image_sharegroup_images_list", tool.Name, "tool name should match")
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Contains(t, textContent.Text, "shared/1", "response should contain image ID")
	})
	...
	t.Run("client error", func(t *testing.T) {
		...
		textContent, ok := result.Content[0].(mcp.TextContent)
		require.True(t, ok, "content should be TextContent")
		assert.Contains(t, textContent.Text, "Failed to retrieve image share group images")
	})
}
