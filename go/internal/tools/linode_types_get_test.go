...
func TestLinodeTypeGetTool(t *testing.T) {
	...
	t.Run("definition", func(t *testing.T) {
		...
		checkEqual(t, linodeTypeGetToolName, tool.Name, "tool name should match")
		checkNotEmpty(t, tool.Description, "tool should have a description")
		checkEqual(t, profiles.CapRead, capability, "tool should declare read capability")
	...
	t.Run("success", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			checkEqual(t, linodeTypeGetPath, r.URL.EscapedPath(), "request path should include type id")
			checkEmpty(t, r.URL.RawQuery, "request query should be empty")
			w.Header().Set("Content-Type", "application/json")
			checkNoError(t, json.NewEncoder(w).Encode(linode.InstanceType{ID: linodeTypeGetID, Label: linodeTypeGetLabel, Class: classStandard}))
		}))
		defer srv.Close()
		...
		checkContains(t, textContent.Text, linodeTypeGetID, "response should contain type id")
		checkContains(t, textContent.Text, linodeTypeGetLabel, "response should contain type label")
	})
}
...
func TestLinodeTypeGetToolValidation(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			checkTrue(t, result.IsError, "invalid type_id should return tool error")
