...
A tool's argument check belongs on its *Input message, as a buf.validate rule,
a declared argument_reader, or an object_walk, all of which both languages
evaluate through one shared reader (go/internal/toolvalidate,
linodemcp.tools.constraints). A check written out in a handler instead is the
debt: two copies of one rule, in two languages, with nothing holding them to the
same sentence. This counts what is left of it.
...
and "the scan sees nothing" is exactly one moved directory.

This population is the one `make hand-code` cannot see. That gate fails by
name on a function named after a TOOL; an argument check is named after what it
checks, so no tool name appears in it and no prefix match reaches it. Planting
`validateResizeTarget` in go/internal/tools and `_validate_resize_target` in
python/src/linodemcp/tools leaves hand-code green and fails this gate. The two
populations do not overlap, which is why both files exist.

Three ways this could pass while measuring nothing are checked first:

- The contract declares no tools at all, which means the descriptors did not
  load and the tree this scans may not be the tree that was built.
- A registered language has no arm here, so its hand-written checks go
  uncounted.
- A language's tree holds no source file, which means the scan stopped seeing
  the tree rather than that the checks are gone.

Which trees are scanned comes from docs/contracts/languages.txt rather than a
path written here.

A handful of names match the scanned spellings and are not per-tool argument
checks at all: the shared page reader, and the bodies two contract members read
a path segment and an object member through. Those are support vocabulary, the
same standing `make_route_request` has, and each language's Tree names them in a
`plumbing` set with the reason written beside it. The set is deliberately small
and deliberately annotated, because it is the one place this gate can be made to
lie; scripts/verify_route_source.py keeps its own for the same reason.

Which name counts as a hand-written check is a naming convention per language,
and the two trees do not spell it the same way. Both trees turn one argument
into either a value or a refusal sentence; Go writes that as `validateX`,
`parseX`, or `XFromTool`, Python as `_validate_x`, `_parse_x`, `_x_error`, or
`_x_argument`. Go's third spelling is the request-builder form, which reads one
or more arguments and returns the value beside a refusal sentence rather than an
error; every one of them ends its result list with that string. Python's fourth
is the argument-reader form, which answers the value or None and leaves its
caller to word the refusal.

Reading only one spelling per tree is how this gate came to be blind to Python's
parse-and-validate helpers, then to Go's request builders, and then to Python's
argument readers, which made migrating a tool that had one read as growth: the
hand-written body it gained counted, the reader it lost did not.
...
_COUNTS_HEADER = (
    ...
    "#\n"
    "# The number is the argument checks a language's hand-written tree still\n"
    ...
    "#\n"
    "# It is the population `make hand-code` cannot see: that gate fails on a\n"
    "# function named after a TOOL, and an argument check is named after what\n"
    "# it checks.\n"
    "#\n"
    "# The population is empty in both languages. Every argument check the\n"
...
class Tree:
    """Where one language keeps its hand-written checks, and how it names them.
...
# Each pattern names every spelling its tree gives an argument check, per the
# module docstring; a helper that only decodes a response body is not one of
# these.
...
def source_files(tree: Tree) -> list[Path]:
    ...
    A tree that holds no file is an error rather than an empty scan, since a
    scan of nothing would pass every check below.
    """
...
def hand_written(tree: Tree) -> list[str]:
    """Every argument check one language's hand-written tree writes out, per site.
...
def measure(
    languages: list[str], sites: dict[str, list[str]]
) -> tuple[dict[str, int], list[str]]:
    """Count each language's hand-written checks, reporting what cannot be counted.

    sites is filled with the definition site of each one counted, so a count
    that comes back over its line can name what pushed it there.
    """
    problems: list[str] = []
    ...
    for language in languages:
        ...
        if tree is None:
            problems.append(
                f"{language} is registered in languages.txt and has no tree"
                " here, so its hand-written checks go uncounted"
            )
            continue

        if not source_files(tree):
            problems.append(
                f"{language}: no source file found under {tree.root}, so the scan"
                " sees nothing rather than a tree with nothing to find"
            )
            continue

        written = hand_written(tree)
        counts[language] = len(written)
        sites[language] = written

    return counts, problems


def compare(
    counts: dict[str, int], recorded: dict[str, int], sites: dict[str, list[str]]
) -> list[str]:
    """Hold each measured count against its recorded line.

    A count over its line names every site it counted, because the number alone
    does not say which check to move onto a message.
    """
    problems: list[str] = []
    ...
    for language, count in counts.items():
        ...
        if count > recorded[language]:
            problems.append(
                ...
                f" tool's *Input message: {', '.join(sites[language])}"
            )
...
def main() -> int:
    ...
    counts, problems = measure(languages, sites)
    ...
    problems = compare(counts, read_counts(_COUNTS), sites)
    if problems:
        for problem in problems:
            sys.stderr.write(f"hand-validators: {problem}\n")
        return 1
